...
#endif
	   search_time = time;
	   printf("%s %d %lld  %lld\n", join->ruleToActivate->header.name->contents,join->ruleToActivate->salience,l_time,r_time);
   ...
   while (rhsBinds != NULL)
     {
	   ...
	   if (rhsBindsIsFact){
		   ...
		   if (factTime < fix_l_time || factTime > fix_r_time)
		   {
			   ...
			   /**/
#if FREEPM  
			   struct partialMatch* lhs_next = lhsBinds->nextInMemory;
			   //if (factTime > r_time) 
			   int useable = TRUE;
			  ...
			  // if (join->numOfActiveNode > 0 && join->activeJoinNodeListTail->currentPartialMatch != NULL)
			   ...
			   {
				   //struct partialMatch* x_PM  = GetAlphaMemory(GetEnvironmentByIndex(1), (struct patternNodeHeader *) last_level_join->rightSideEntryStructure, entryHashValue);
